...
```js
    ...
    */
    /*
    * 使用闭包，是为了实现单独作用域，可以保留对象的引用，为了新消息监听回调绑定
    * loginInfo 登陆所需的参数，详情参考官方文档
    * MsgNotifyCallBack 当有最新消息时所触发的回调函数
    * 目前只监听 新消息和连接状态，就可以实现简单1v1聊天
    * promise 封装，im 需要监听多种异步，所以配合 async/await 函数
    */
    let login = (function(){
        // 绑定 onMsgNotify this 指向 / 因为im登陆后绑定监听事件函数，重新赋值，所以绑定对象指针，动态修改cb函数，实现多页cb
        let MsgNotifyObj = {};
